class WeaponRunner {

    public static void main(String[] args) {

        System.out.println("Main Started");

        Weapon weapon1 = new Weapon(1, "Desert Eagle", "Pistol", "Steel", 1.8, 26.5, true);
        System.out.println("=== Weapon 1 ===");
        weapon1.getWeaponInfo();

        Weapon weapon2 = new Weapon(2, "Katana", "Sword", "Carbon Steel", 1.2, 100.0, false);
        System.out.println("=== Weapon 2 ===");
        weapon2.getWeaponInfo();

        Weapon weapon3 = new Weapon(3, "AK-47", "Rifle", "Steel", 4.3, 87.0, true);
        System.out.println("=== Weapon 3 ===");
        weapon3.getWeaponInfo();

        Weapon weapon4 = new Weapon(4, "Crossbow", "Bow", "Fiberglass", 2.5, 78.0, false);
        System.out.println("=== Weapon 4 ===");
        weapon4.getWeaponInfo();

        Weapon weapon5 = new Weapon(5, "Trident", "Spear", "Iron", 3.0, 150.0, false);
        System.out.println("=== Weapon 5 ===");
        weapon5.getWeaponInfo();

        Weapon weapon6 = new Weapon(6, "Machete", "Blade", "Stainless Steel", 1.0, 60.0, false);
        System.out.println("=== Weapon 6 ===");
        weapon6.getWeaponInfo();

        Weapon weapon7 = new Weapon(7, "Nunchaku", "Martial Arts", "Wood", 0.5, 30.0, false);
        System.out.println("=== Weapon 7 ===");
        weapon7.getWeaponInfo();

        Weapon weapon8 = new Weapon(8, "Grenade", "Explosive", "Metal", 0.4, 10.0, true);
        System.out.println("=== Weapon 8 ===");
        weapon8.getWeaponInfo();

        Weapon weapon9 = new Weapon(9, "Taser", "Electroshock", "Plastic & Metal", 0.6, 15.0, true);
        System.out.println("=== Weapon 9 ===");
        weapon9.getWeaponInfo();

        Weapon weapon10 = new Weapon(10, "Tomahawk", "Axe", "Carbon Steel", 1.5, 50.0, false);
        System.out.println("=== Weapon 10 ===");
        weapon10.getWeaponInfo();

        Weapon weapon11 = new Weapon(11, "Sickle", "Sharp Tool", "Iron", 0.7, 35.0, false);
        System.out.println("=== Weapon 11 ===");
        weapon11.getWeaponInfo();

        Weapon weapon12 = new Weapon(12, "Throwing Star", "Shuriken", "Stainless Steel", 0.2, 12.0, false);
        System.out.println("=== Weapon 12 ===");
        weapon12.getWeaponInfo();

        Weapon weapon13 = new Weapon(13, "Morning Star", "Spiked Mace", "Iron", 3.5, 80.0, false);
        System.out.println("=== Weapon 13 ===");
        weapon13.getWeaponInfo();

        Weapon weapon14 = new Weapon(14, "Flamethrower", "Incendiary", "Metal", 15.0, 110.0, true);
        System.out.println("=== Weapon 14 ===");
        weapon14.getWeaponInfo();

        Weapon weapon15 = new Weapon(15, "Whip", "Flexible", "Leather", 0.9, 180.0, false);
        System.out.println("=== Weapon 15 ===");
        weapon15.getWeaponInfo();

        Weapon weapon16 = new Weapon(16, "Bayonet", "Knife", "Steel", 0.8, 35.0, true);
        System.out.println("=== Weapon 16 ===");
        weapon16.getWeaponInfo();

        Weapon weapon17 = new Weapon(17, "Spear", "Polearm", "Steel and Wood", 2.8, 220.0, false);
        System.out.println("=== Weapon 17 ===");
        weapon17.getWeaponInfo();

        Weapon weapon18 = new Weapon(18, "Slingshot", "Projectile", "Plastic", 0.3, 18.0, false);
        System.out.println("=== Weapon 18 ===");
        weapon18.getWeaponInfo();

        Weapon weapon19 = new Weapon(19, "Laser Gun", "Sci-Fi", "Futuristic Alloy", 5.0, 95.0, true);
        System.out.println("=== Weapon 19 ===");
        weapon19.getWeaponInfo();

        Weapon weapon20 = new Weapon(20, "Boomerang", "Returning Throw", "Wood", 0.4, 25.0, false);
        System.out.println("=== Weapon 20 ===");
        weapon20.getWeaponInfo();

        System.out.println("Main Ended");
    }
}



        /*Weapon weapon1 = new Weapon();
        weapon1.weaponId = 1;
        weapon1.name = "Desert Eagle";
        weapon1.type = "Pistol";
        weapon1.material = "Steel";
        weapon1.weight = 1.8;
        weapon1.length = 26.5;
        weapon1.isLicensed = true;
        System.out.println("=== Weapon 1 ===");
        weapon1.getWeaponInfo();

        Weapon weapon2 = new Weapon();
        weapon2.weaponId = 2;
        weapon2.name = "Katana";
        weapon2.type = "Sword";
        weapon2.material = "Carbon Steel";
        weapon2.weight = 1.2;
        weapon2.length = 100.0;
        weapon2.isLicensed = false;
        System.out.println("=== Weapon 2 ===");
        weapon2.getWeaponInfo();

        Weapon weapon3 = new Weapon();
        weapon3.weaponId = 3;
        weapon3.name = "AK-47";
        weapon3.type = "Rifle";
        weapon3.material = "Steel";
        weapon3.weight = 4.3;
        weapon3.length = 87.0;
        weapon3.isLicensed = true;
        System.out.println("=== Weapon 3 ===");
        weapon3.getWeaponInfo();

        Weapon weapon4 = new Weapon();
        weapon4.weaponId = 4;
        weapon4.name = "Crossbow";
        weapon4.type = "Bow";
        weapon4.material = "Fiberglass";
        weapon4.weight = 2.5;
        weapon4.length = 78.0;
        weapon4.isLicensed = false;
        System.out.println("=== Weapon 4 ===");
        weapon4.getWeaponInfo();

        Weapon weapon5 = new Weapon();
        weapon5.weaponId = 5;
        weapon5.name = "Trident";
        weapon5.type = "Spear";
        weapon5.material = "Iron";
        weapon5.weight = 3.0;
        weapon5.length = 150.0;
        weapon5.isLicensed = false;
        System.out.println("=== Weapon 5 ===");
        weapon5.getWeaponInfo();

        Weapon weapon6 = new Weapon();
        weapon6.weaponId = 6;
        weapon6.name = "Machete";
        weapon6.type = "Blade";
        weapon6.material = "Stainless Steel";
        weapon6.weight = 1.0;
        weapon6.length = 60.0;
        weapon6.isLicensed = false;
        System.out.println("=== Weapon 6 ===");
        weapon6.getWeaponInfo();

        Weapon weapon7 = new Weapon();
        weapon7.weaponId = 7;
        weapon7.name = "Nunchaku";
        weapon7.type = "Martial Arts";
        weapon7.material = "Wood";
        weapon7.weight = 0.5;
        weapon7.length = 30.0;
        weapon7.isLicensed = false;
        System.out.println("=== Weapon 7 ===");
        weapon7.getWeaponInfo();

        Weapon weapon8 = new Weapon();
        weapon8.weaponId = 8;
        weapon8.name = "Grenade";
        weapon8.type = "Explosive";
        weapon8.material = "Metal";
        weapon8.weight = 0.4;
        weapon8.length = 10.0;
        weapon8.isLicensed = true;
        System.out.println("=== Weapon 8 ===");
        weapon8.getWeaponInfo();

        Weapon weapon9 = new Weapon();
        weapon9.weaponId = 9;
        weapon9.name = "Taser";
        weapon9.type = "Electroshock";
        weapon9.material = "Plastic & Metal";
        weapon9.weight = 0.6;
        weapon9.length = 15.0;
        weapon9.isLicensed = true;
        System.out.println("=== Weapon 9 ===");
        weapon9.getWeaponInfo();

        Weapon weapon10 = new Weapon();
        weapon10.weaponId = 10;
        weapon10.name = "Tomahawk";
        weapon10.type = "Axe";
        weapon10.material = "Carbon Steel";
        weapon10.weight = 1.5;
        weapon10.length = 50.0;
        weapon10.isLicensed = false;
        System.out.println("=== Weapon 10 ===");
        weapon10.getWeaponInfo();

        Weapon weapon11 = new Weapon();
        weapon11.weaponId = 11;
        weapon11.name = "Sickle";
        weapon11.type = "Sharp Tool";
        weapon11.material = "Iron";
        weapon11.weight = 0.7;
        weapon11.length = 35.0;
        weapon11.isLicensed = false;
        System.out.println("=== Weapon 11 ===");
        weapon11.getWeaponInfo();

        Weapon weapon12 = new Weapon();
        weapon12.weaponId = 12;
        weapon12.name = "Throwing Star";
        weapon12.type = "Shuriken";
        weapon12.material = "Stainless Steel";
        weapon12.weight = 0.2;
        weapon12.length = 12.0;
        weapon12.isLicensed = false;
        System.out.println("=== Weapon 12 ===");
        weapon12.getWeaponInfo();

        Weapon weapon13 = new Weapon();
        weapon13.weaponId = 13;
        weapon13.name = "Morning Star";
        weapon13.type = "Spiked Mace";
        weapon13.material = "Iron";
        weapon13.weight = 3.5;
        weapon13.length = 80.0;
        weapon13.isLicensed = false;
        System.out.println("=== Weapon 13 ===");
        weapon13.getWeaponInfo();

        Weapon weapon14 = new Weapon();
        weapon14.weaponId = 14;
        weapon14.name = "Flamethrower";
        weapon14.type = "Incendiary";
        weapon14.material = "Metal";
        weapon14.weight = 15.0;
        weapon14.length = 110.0;
        weapon14.isLicensed = true;
        System.out.println("=== Weapon 14 ===");
        weapon14.getWeaponInfo();

        Weapon weapon15 = new Weapon();
        weapon15.weaponId = 15;
        weapon15.name = "Whip";
        weapon15.type = "Flexible";
        weapon15.material = "Leather";
        weapon15.weight = 0.9;
        weapon15.length = 180.0;
        weapon15.isLicensed = false;
        System.out.println("=== Weapon 15 ===");
        weapon15.getWeaponInfo();

        Weapon weapon16 = new Weapon();
        weapon16.weaponId = 16;
        weapon16.name = "Bayonet";
        weapon16.type = "Knife";
        weapon16.material = "Steel";
        weapon16.weight = 0.8;
        weapon16.length = 35.0;
        weapon16.isLicensed = true;
        System.out.println("=== Weapon 16 ===");
        weapon16.getWeaponInfo();

        Weapon weapon17 = new Weapon();
        weapon17.weaponId = 17;
        weapon17.name = "Spear";
        weapon17.type = "Polearm";
        weapon17.material = "Steel and Wood";
        weapon17.weight = 2.8;
        weapon17.length = 220.0;
        weapon17.isLicensed = false;
        System.out.println("=== Weapon 17 ===");
        weapon17.getWeaponInfo();

        Weapon weapon18 = new Weapon();
        weapon18.weaponId = 18;
        weapon18.name = "Slingshot";
        weapon18.type = "Projectile";
        weapon18.material = "Plastic";
        weapon18.weight = 0.3;
        weapon18.length = 18.0;
        weapon18.isLicensed = false;
        System.out.println("=== Weapon 18 ===");
        weapon18.getWeaponInfo();

        Weapon weapon19 = new Weapon();
        weapon19.weaponId = 19;
        weapon19.name = "Laser Gun";
        weapon19.type = "Sci-Fi";
        weapon19.material = "Futuristic Alloy";
        weapon19.weight = 5.0;
        weapon19.length = 95.0;
        weapon19.isLicensed = true;
        System.out.println("=== Weapon 19 ===");
        weapon19.getWeaponInfo();

        Weapon weapon20 = new Weapon();
        weapon20.weaponId = 20;
        weapon20.name = "Boomerang";
        weapon20.type = "Returning Throw";
        weapon20.material = "Wood";
        weapon20.weight = 0.4;
        weapon20.length = 25.0;
        weapon20.isLicensed = false;
        System.out.println("=== Weapon 20 ===");
        weapon20.getWeaponInfo();*/
class GymEquipmentRunner {

    public static void main(String[] args) {

        System.out.println("Main Started");

        GymEquipment equipment1 = new GymEquipment(1, "Treadmill", "FitTech", "Cardio", 80.5, 55000.0, true);
        System.out.println("=== Equipment 1 ===");
        equipment1.getGymEquipmentInfo();

        GymEquipment equipment2 = new GymEquipment(2, "Elliptical", "HealthMax", "Cardio", 65.0, 48000.0, true);
        System.out.println("=== Equipment 2 ===");
        equipment2.getGymEquipmentInfo();

        GymEquipment equipment3 = new GymEquipment(3, "Dumbbell Set", "IronGrip", "Strength", 20.0, 3200.0, false);
        System.out.println("=== Equipment 3 ===");
        equipment3.getGymEquipmentInfo();

        GymEquipment equipment4 = new GymEquipment(4, "Rowing Machine", "ProRow", "Cardio", 75.0, 60000.0, true);
        System.out.println("=== Equipment 4 ===");
        equipment4.getGymEquipmentInfo();

        GymEquipment equipment5 = new GymEquipment(5, "Bench Press", "MuscleForge", "Strength", 50.0, 25000.0, false);
        System.out.println("=== Equipment 5 ===");
        equipment5.getGymEquipmentInfo();

        GymEquipment equipment6 = new GymEquipment(6, "Exercise Bike", "CycleFit", "Cardio", 42.0, 27000.0, true);
        System.out.println("=== Equipment 6 ===");
        equipment6.getGymEquipmentInfo();

        GymEquipment equipment7 = new GymEquipment(7, "Kettlebells", "PowerBell", "Strength", 16.0, 4500.0, false);
        System.out.println("=== Equipment 7 ===");
        equipment7.getGymEquipmentInfo();

        GymEquipment equipment8 = new GymEquipment(8, "Leg Press", "LegMaster", "Strength", 90.0, 72000.0, false);
        System.out.println("=== Equipment 8 ===");
        equipment8.getGymEquipmentInfo();

        GymEquipment equipment9 = new GymEquipment(9, "Smith Machine", "IronSmith", "Strength", 100.0, 85000.0, false);
        System.out.println("=== Equipment 9 ===");
        equipment9.getGymEquipmentInfo();

        GymEquipment equipment10 = new GymEquipment(10, "Pull-up Bar", "FlexBar", "Strength", 10.0, 1500.0, false);
        System.out.println("=== Equipment 10 ===");
        equipment10.getGymEquipmentInfo();

        GymEquipment equipment11 = new GymEquipment(11, "Power Rack", "RackPro", "Strength", 120.0, 90000.0, false);
        System.out.println("=== Equipment 11 ===");
        equipment11.getGymEquipmentInfo();

        GymEquipment equipment12 = new GymEquipment(12, "Cable Machine", "CableX", "Strength", 95.0, 78000.0, false);
        System.out.println("=== Equipment 12 ===");
        equipment12.getGymEquipmentInfo();

        GymEquipment equipment13 = new GymEquipment(13, "Punching Bag", "BoxPro", "Conditioning", 30.0, 5000.0, false);
        System.out.println("=== Equipment 13 ===");
        equipment13.getGymEquipmentInfo();

        GymEquipment equipment14 = new GymEquipment(14, "Medicine Ball", "MedFit", "Conditioning", 10.0, 1200.0, false);
        System.out.println("=== Equipment 14 ===");
        equipment14.getGymEquipmentInfo();

        GymEquipment equipment15 = new GymEquipment(15, "Lat Pulldown", "LatKing", "Strength", 85.0, 69000.0, false);
        System.out.println("=== Equipment 15 ===");
        equipment15.getGymEquipmentInfo();

        GymEquipment equipment16 = new GymEquipment(16, "Battle Ropes", "RopeX", "Conditioning", 12.0, 3000.0, false);
        System.out.println("=== Equipment 16 ===");
        equipment16.getGymEquipmentInfo();

        GymEquipment equipment17 = new GymEquipment(17, "Resistance Bands", "BandFlex", "Strength", 2.0, 800.0, false);
        System.out.println("=== Equipment 17 ===");
        equipment17.getGymEquipmentInfo();

        GymEquipment equipment18 = new GymEquipment(18, "Ab Roller", "CoreMaster", "Strength", 3.0, 1000.0, false);
        System.out.println("=== Equipment 18 ===");
        equipment18.getGymEquipmentInfo();

        GymEquipment equipment19 = new GymEquipment(19, "Vibration Plate", "VibeFit", "Cardio", 30.0, 25000.0, true);
        System.out.println("=== Equipment 19 ===");
        equipment19.getGymEquipmentInfo();

        GymEquipment equipment20 = new GymEquipment(20, "Stepper Machine", "StepStrong", "Cardio", 48.0, 39000.0, true);
        System.out.println("=== Equipment 20 ===");
        equipment20.getGymEquipmentInfo();

        System.out.println("Main Ended");
    }
}



        /*GymEquipment equipment1 = new GymEquipment();
        equipment1.equipmentId = 1;
        equipment1.name = "Treadmill";
        equipment1.brand = "FitTech";
        equipment1.category = "Cardio";
        equipment1.weight = 80.5;
        equipment1.price = 55000.0;
        equipment1.isElectric = true;
        System.out.println("=== Equipment 1 ===");
        equipment1.getGymEquipmentInfo();

        GymEquipment equipment2 = new GymEquipment();
        equipment2.equipmentId = 2;
        equipment2.name = "Elliptical";
        equipment2.brand = "HealthMax";
        equipment2.category = "Cardio";
        equipment2.weight = 65.0;
        equipment2.price = 48000.0;
        equipment2.isElectric = true;
        System.out.println("=== Equipment 2 ===");
        equipment2.getGymEquipmentInfo();

        GymEquipment equipment3 = new GymEquipment();
        equipment3.equipmentId = 3;
        equipment3.name = "Dumbbell Set";
        equipment3.brand = "IronGrip";
        equipment3.category = "Strength";
        equipment3.weight = 20.0;
        equipment3.price = 3200.0;
        equipment3.isElectric = false;
        System.out.println("=== Equipment 3 ===");
        equipment3.getGymEquipmentInfo();

        GymEquipment equipment4 = new GymEquipment();
        equipment4.equipmentId = 4;
        equipment4.name = "Rowing Machine";
        equipment4.brand = "ProRow";
        equipment4.category = "Cardio";
        equipment4.weight = 75.0;
        equipment4.price = 60000.0;
        equipment4.isElectric = true;
        System.out.println("=== Equipment 4 ===");
        equipment4.getGymEquipmentInfo();

        GymEquipment equipment5 = new GymEquipment();
        equipment5.equipmentId = 5;
        equipment5.name = "Bench Press";
        equipment5.brand = "MuscleForge";
        equipment5.category = "Strength";
        equipment5.weight = 50.0;
        equipment5.price = 25000.0;
        equipment5.isElectric = false;
        System.out.println("=== Equipment 5 ===");
        equipment5.getGymEquipmentInfo();

        GymEquipment equipment6 = new GymEquipment();
        equipment6.equipmentId = 6;
        equipment6.name = "Exercise Bike";
        equipment6.brand = "CycleFit";
        equipment6.category = "Cardio";
        equipment6.weight = 42.0;
        equipment6.price = 27000.0;
        equipment6.isElectric = true;
        System.out.println("=== Equipment 6 ===");
        equipment6.getGymEquipmentInfo();

        GymEquipment equipment7 = new GymEquipment();
        equipment7.equipmentId = 7;
        equipment7.name = "Kettlebells";
        equipment7.brand = "PowerBell";
        equipment7.category = "Strength";
        equipment7.weight = 16.0;
        equipment7.price = 4500.0;
        equipment7.isElectric = false;
        System.out.println("=== Equipment 7 ===");
        equipment7.getGymEquipmentInfo();

        GymEquipment equipment8 = new GymEquipment();
        equipment8.equipmentId = 8;
        equipment8.name = "Leg Press";
        equipment8.brand = "LegMaster";
        equipment8.category = "Strength";
        equipment8.weight = 90.0;
        equipment8.price = 72000.0;
        equipment8.isElectric = false;
        System.out.println("=== Equipment 8 ===");
        equipment8.getGymEquipmentInfo();

        GymEquipment equipment9 = new GymEquipment();
        equipment9.equipmentId = 9;
        equipment9.name = "Smith Machine";
        equipment9.brand = "IronSmith";
        equipment9.category = "Strength";
        equipment9.weight = 100.0;
        equipment9.price = 85000.0;
        equipment9.isElectric = false;
        System.out.println("=== Equipment 9 ===");
        equipment9.getGymEquipmentInfo();

        GymEquipment equipment10 = new GymEquipment();
        equipment10.equipmentId = 10;
        equipment10.name = "Pull-up Bar";
        equipment10.brand = "FlexBar";
        equipment10.category = "Strength";
        equipment10.weight = 10.0;
        equipment10.price = 1500.0;
        equipment10.isElectric = false;
        System.out.println("=== Equipment 10 ===");
        equipment10.getGymEquipmentInfo();

        GymEquipment equipment11 = new GymEquipment();
        equipment11.equipmentId = 11;
        equipment11.name = "Power Rack";
        equipment11.brand = "RackPro";
        equipment11.category = "Strength";
        equipment11.weight = 120.0;
        equipment11.price = 90000.0;
        equipment11.isElectric = false;
        System.out.println("=== Equipment 11 ===");
        equipment11.getGymEquipmentInfo();

        GymEquipment equipment12 = new GymEquipment();
        equipment12.equipmentId = 12;
        equipment12.name = "Cable Machine";
        equipment12.brand = "CableX";
        equipment12.category = "Strength";
        equipment12.weight = 95.0;
        equipment12.price = 78000.0;
        equipment12.isElectric = false;
        System.out.println("=== Equipment 12 ===");
        equipment12.getGymEquipmentInfo();

        GymEquipment equipment13 = new GymEquipment();
        equipment13.equipmentId = 13;
        equipment13.name = "Punching Bag";
        equipment13.brand = "BoxPro";
        equipment13.category = "Conditioning";
        equipment13.weight = 30.0;
        equipment13.price = 5000.0;
        equipment13.isElectric = false;
        System.out.println("=== Equipment 13 ===");
        equipment13.getGymEquipmentInfo();

        GymEquipment equipment14 = new GymEquipment();
        equipment14.equipmentId = 14;
        equipment14.name = "Medicine Ball";
        equipment14.brand = "MedFit";
        equipment14.category = "Conditioning";
        equipment14.weight = 10.0;
        equipment14.price = 1200.0;
        equipment14.isElectric = false;
        System.out.println("=== Equipment 14 ===");
        equipment14.getGymEquipmentInfo();

        GymEquipment equipment15 = new GymEquipment();
        equipment15.equipmentId = 15;
        equipment15.name = "Lat Pulldown";
        equipment15.brand = "LatKing";
        equipment15.category = "Strength";
        equipment15.weight = 85.0;
        equipment15.price = 69000.0;
        equipment15.isElectric = false;
        System.out.println("=== Equipment 15 ===");
        equipment15.getGymEquipmentInfo();

        GymEquipment equipment16 = new GymEquipment();
        equipment16.equipmentId = 16;
        equipment16.name = "Battle Ropes";
        equipment16.brand = "RopeX";
        equipment16.category = "Conditioning";
        equipment16.weight = 12.0;
        equipment16.price = 3000.0;
        equipment16.isElectric = false;
        System.out.println("=== Equipment 16 ===");
        equipment16.getGymEquipmentInfo();

        GymEquipment equipment17 = new GymEquipment();
        equipment17.equipmentId = 17;
        equipment17.name = "Resistance Bands";
        equipment17.brand = "BandFlex";
        equipment17.category = "Strength";
        equipment17.weight = 2.0;
        equipment17.price = 800.0;
        equipment17.isElectric = false;
        System.out.println("=== Equipment 17 ===");
        equipment17.getGymEquipmentInfo();

        GymEquipment equipment18 = new GymEquipment();
        equipment18.equipmentId = 18;
        equipment18.name = "Ab Roller";
        equipment18.brand = "CoreMaster";
        equipment18.category = "Strength";
        equipment18.weight = 3.0;
        equipment18.price = 1000.0;
        equipment18.isElectric = false;
        System.out.println("=== Equipment 18 ===");
        equipment18.getGymEquipmentInfo();

        GymEquipment equipment19 = new GymEquipment();
        equipment19.equipmentId = 19;
        equipment19.name = "Vibration Plate";
        equipment19.brand = "VibeFit";
        equipment19.category = "Cardio";
        equipment19.weight = 30.0;
        equipment19.price = 25000.0;
        equipment19.isElectric = true;
        System.out.println("=== Equipment 19 ===");
        equipment19.getGymEquipmentInfo();

        GymEquipment equipment20 = new GymEquipment();
        equipment20.equipmentId = 20;
        equipment20.name = "Stepper Machine";
        equipment20.brand = "StepStrong";
        equipment20.category = "Cardio";
        equipment20.weight = 48.0;
        equipment20.price = 39000.0;
        equipment20.isElectric = true;
        System.out.println("=== Equipment 20 ===");
        equipment20.getGymEquipmentInfo();*/